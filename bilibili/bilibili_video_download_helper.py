...
def do_prepare(inputStart, inputQuality):
    ...
    start_time = time.time()
    # 用户输入av号或者视频链接地址
    print('*' * 30 + 'MoriiHuang的B站视频下载小助手' + '*' * 30)
    start = inputStart
    if start.isdigit() == True:  # 如果输入的是av号
        # 获取cid的api, 传入aid即可
        start_url = 'https://api.bilibili.com/x/web-interface/view?aid=' + start
    elif(re.search(r'/av(\d+)/*', start)!=None):
        ...
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=BV' + re.search(r'/BV(\w+)/*', start).group(1)
    # 视频质量
...
if __name__ == "__main__":
    # 设置标题
    root.title('MoriiHuang的B站视频下载小助手-GUI')
    inputStart = Entry(root, bd=4, width=600)
    labelStart = Label(root, text="请输入您要下载的B站av号或者BV号视频链接或者视频链接地址:")  # 地址输入
    ...
    msgbox = Text(root)
    msgbox.insert('1.0',
                  "对于单P视频:直接传入B站av号或者BV号完整视频链接地址或者av视频链接地址\n(eg: 314或者https://www.bilibili.com/video/BV1Gt411b7Zq（切记带上BV）或者https://www.bilibili.com/video/av314)\n对于多P视频:\n1.下载全集:直接传入B站av号或者视频链接地址\n(eg: 314或者https://www.bilibili.com/video/av314)\n2.下载其中一集:传入那一集的视频链接地址\n")
    msgbox.pack()
